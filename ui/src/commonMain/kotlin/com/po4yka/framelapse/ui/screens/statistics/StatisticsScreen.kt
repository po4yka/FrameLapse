...
private fun StatisticsContent(
    ...
    Scaffold(
        ...
        Column(
            ...
            // Using local variables for cross-module smart cast compatibility
            val projectStats = state.projectStats
            val globalStats = state.globalStats
            val error = state.error

            when {
                state.isLoading -> LoadingIndicator()
                state.mode == StatisticsMode.PROJECT && projectStats != null -> {
                    ProjectStatsContent(
                        stats = projectStats,
                        modifier = Modifier
                            .fillMaxSize()
                ...
                state.mode == StatisticsMode.GLOBAL && globalStats != null -> {
                    GlobalStatsContent(
                        stats = globalStats,
                        modifier = Modifier
                            .fillMaxSize()
                ...
                error != null -> {
                    EmptyStatsContent(message = error)
                }
